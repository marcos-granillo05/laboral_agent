...
formatter_instructions_v03 = """
    ...
    Debes responder únicamente con un JSON válido que cumpla exactamente este esquema:
    {
        "personal_data_content": "Toda la información relacionada al trabajador.",
        "content": "toda la información relacionada al relato de hecho, e información extraída de documentos",
    ...
    - `personal_data_content` debe contener solo los datos personales y laborales del trabajador.
    - `content` debe contener solo el relato de hechos.
    - `claims_statement_facts` Identificación de pretensiones sobre relato de hecho, NO OMITAS DETALLES. 
    """
# --------------------------------------------------- Formatter Instructions ---------------------------------------------------------------
...
eval_service_instruction="""
 ...
 # Manejo de información
    - Recibirás información del agente 'laboral_privado_formatter' con el siguiente formato JSON
      {
        "personal_data_content": "Resumen completo de los datos personales y laborales del trabajador.",
        "content": "Información asociada al relato de ls hechos",
        "documents_related": "Información extraída de los documentos". 
    }
    Ese input lo recibirás a través de {laboral_privado_formatter_output}

 # INSTRUCCIONES. 
    - De la información pasada por el agente 'laboral_privado_formatter', te centrarás específicamente en la información de "content" , para complementar 
    ...
    - Usa `personal_data_content` solo cuando sea necesario para completar el diagnóstico, la temporalidad, la cuantía o el trámite sugerido.
...
legal_advisor_instructions = """
    ...
    - Para la correcta interpretación de Leyes, en el ámbito de derecho laboral de sector privado deberás seguir el orden.
    PROCESO ESTÁNDAR.
        1. Análisis de leyes propiamente de sector privado:
            - Sección : Sector Privado
        2. Análisis de leyes internacionales o aplicables a sector privado.
    ...
    # Sector Privado 
     Para confirmar que el relato es de naturaleza laboral, debes validar la presencia de tres elementos clave:
    ...
    Para el contraste con leyes, tienes acceso a las tools:
        ...
        - rag_laboral_privado: Conjunto de leyes asociadas al sector privado.
            - Cubre la sección: Sector privado
...
# Entrada:
    - Recibirás la información del agente 'laboral_privado_legal_advisor' con esta estructura:
   {
        "personal_data_summary": "Resumen de información personal del trabajador",
        "personal_data": {
            "name": "Nombre del trabajador",
    ...
    Ese input lo recibirás a través de {laboral_privado_legal_advisor_output}.

    - Recibirás información del agente 'laboral_privado_eval_service' con esta estructura:
    {
        "legal_diagnosis": "Trámite específico que el trabajador debería optar",
    ...
    Ese input lo recibirás a través de {laboral_privado_eval_service_output}.
...
legal_advisor_instructions_v02 = """
    ...
    # INSTRUCCIONES. 
    - De la información pasada por el agente 'laboral_privado_formatter', te centrarás específicamente en la información de "content" , para complementar 
    "claims_statement_facts" que tiene información complementaria sobre la pretensión identificada del caso, para que puedas realizar un
    análisis detallado en función de que leyes se le han sido violadas al trabajador. Dichos parámetros los encontrarás en la sección de Parámetros de análisis" 
...
reporter_instructions = """
    ...
    # Entrada
    - Recibirás la información del agente 'laboral_privado_response_joiner' con esta estructura:
    {
        "personal_data_summary": "Resumen de información personal del trabajador",
        "personal_data": {
            "name": "Nombre del trabajador",
    ...
    - El input lo recibirás a través de {laboral_privado_response_joiner_output}.

    # Uso de herramienta
    - Usa la herramienta `document_maker`.
    - Debes enviar a `document_maker` un único argumento llamado `analysis_json`.
    - El valor de `analysis_json` debe contener exactamente la información recibida desde `laboral_privado_response_joiner_output`, serializada como JSON válido, con comillas dobles y sin texto adicional.
    ...
    - Debes mapear todos los campos desde `laboral_privado_response_joiner_output`. y generar una estructura JSON tal que así: 
    {
    "personal_data_summary": "Resumen de información personal del trabajador."
    personal_data: : {
            "name": "Nombre del trabajador",
...
reporter_instructions_v02 = """
    ...
    - El input lo recibirás a través de {laboral_privado_legal_advisor_output}.

    # Uso de herramienta
    - Usa la herramienta `document_maker`.
    - Debes enviar a `document_maker` un único argumento llamado `analysis_json`.
    - El valor de `analysis_json` debe contener exactamente la información recibida desde `laboral_privado_legal_advisor_output`, serializada como JSON válido, 
      con comillas dobles y sin texto adicional.
    - No envíes explicaciones, markdown, prefijos ni más de un objeto JSON.

    # Formato de salida
    - Debes responder únicamente con un JSON válido que cumpla exactamente la estructura definida en `reporterOutputSchema_v02`.
    - Debes mapear todos los campos desde `laboral_privado_legal_advisor_output`. y generar una estructura JSON tal que así: 
    {
    "personal_data_summary": "Resumen de información personal del trabajador."
    personal_data: : {
            "name": "Nombre del trabajador",
